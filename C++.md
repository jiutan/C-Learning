...
2. 保护继承（protect）：== 父类中 public 和 protected 的内容 继承到 子类的 protected 中==
  - 父类中 **private** 的内容 ：==**无法继承**==
...
3. 注意： 多继承 可能 会引发 **父类中有同名成员出现**，需要==**加 作用域 区分**==
4. C++世纪开发中，不建议 用 多继承

#### 7.6.9 菱形 继承
1. 概念：
  - 两个 派生类B,C  继承 同一个 基类A
  - 又有 某个类D 同时 继承 这 两个派生类B,C
这种 继承的 方式：称为==**菱形继承/钻石继承**==
2. 问题：
  1. B，C 两类继承了 A类 的数据，当 D类 使用 数据时，会产生**二义性**
  2. 当 D类 继承 来自A类的数据 时，继承了两份（**资源浪费**）。但我们 只需要 继承一份数据
3. 答案：
  1. 二义性 问题：使用==**作用域**来 区分==
  2. 资源浪费 问题：使用==**虚继承virtual**==
4. ==虚继承virtual==：
  - 位置：在两个**派生类**上 使用 **虚继承**
  - 语法：定义派生类时，用 **`class 派生类名 : virtual 继承方式 基类名{};`**
  - 作用：使 基类 与 派生类 中 同名成员，用**基类 中 的 同名成员 代替**(只有 一份)
5. 原理：使用 虚继承 后
  - 派生类 的 同名成员 变为：**虚基类指针`vbptr`**
  - 虚基类指针vbptr 指向 **虚基类表`vbtable`**
  - 其中，虚基类表vbtable 中 存放的 是 地址 与 偏移量。
  - 通过 ==虚基类指针 指向 **地址+偏移量** 指向 **同一份成员**==

### 7.7 多态【三大特性之一】
1. 多态 分为 两类：
  - 静态多态：**函数重载**和**运算符重载**属于 静态多态（复用函数名）
  - 动态多态（默认）：**派生类**和**虚函数** 实现 运行时 的 多态
2. 静态多态 和 动态多态 区别：
  - 静态多态：==函数地址**早绑定 - 编译阶段** 确定 函数地址==
  - 动态多态：==函数地址**晚绑定 - 运行阶段** 确定 函数地址==
3. 注意：
  - C++允许 **父类 与 子类 之间的类型转换**：父类的引用/指针 可以指向 子类对象
4. 作用:
- 多态体现在==父类指针 通过 指向不同子类对象 实现 不同的功能，**相同函数 不同功能**==
#### 7.7.1 动态多态 （运行时 绑定）
##### 动态多态 的 满足条件
1. 有 继承关系
2. ==子类  **重写** **父类** 中的 **虚函数**
...
1. 位置：在 父类 中
2. 语法：
- 父类 内部 声明：**`virtual ~类名() = 0;`**
- 父类 外部 定义：**`类名::类名(){}`**
3. 注意：纯虚析构 需要有 ==声明 和 具体实现== 的，**二者缺一不可**
  - 原因：有可能 父类 有些属性 要 开辟到 堆区
##### 总结：
1. 虚析构 或 纯虚析构 是用来 解决 通过 父类指针 释放 子类对象 的
2. 如果 ==子类中 **没有 堆区数据**，可以 不写成 虚析构或纯虚析构==
3. 拥有 纯虚析构函数 的 类，也属于 **抽象类**
...
## 八、模板 【思想：泛型编程】
### 8.1 模板的概念
1. 概念：模板 就是 建立**通用的模具**，大大的提高 复用性
2. 特点：
  - 模板 不可以 直接使用，它只是一个框架
  - 模板的通用 并不是 万能的
3. C++中 提供两种 模板机制：**函数模板 和 类模板**
### 8.2 函数模板
1. 作用：建立一个通用函数，其 函数的**返回值和形参类型 可以 不具体指定**，用一个==虚拟的类型 T==来代表。
2. 意义：将**类型 参数化**，提高 复用性
#### 8.2.1  声明/定义 函数模板
1. 语法：
	函数的声明 或 定义：
```c
	template<typename T>
	函数声明 或 定义（使用 T 代替 形参类型）
```
2. 解释：
- 【==关键字==】template：声明 创建模板
- 【==关键字==】typename：表明 其后面的符号 是一种数据类型（可以用 class 代替）
...
3. 注意事项：
- 使用 自动类型推导 时：必须 推导出 **一致的数据类型 T**，才可以 使用
- 模板 必须要 确定出 T 的 数据类型，才可以使用
```c
template<typename T>
void func(){
	cout << "func调用" << endl;
...
void test(){
	func();				// 错误： 编译器 无法 确定 T 的类型（没有模板时，正确）
	
	func<int>();		// 正确： 随便给一个数据类型，即可运行
}
```
...
#### 8.3.5 类模板 与 继承
1. 方法一：
当 **子类继承父类的 是一个类模板** 时，子类**在声明时，要指定出 父类中T的类型**
  - 如果 不指定父类中T的 类型，编译器无法给 子类分配内存。
```c
// 父类 模板
template<class T>
class Father{
...
// 子类 继承
class Son:public Father<指定类型>{
};
```
2. 方法二： 如果想 **灵活指定出 父类中T的类型，子类也需要变为 类模板**。
在 **实例化子类对象 时，需 声明 T的类型**
```c
// 父类 模板
template<class T>
class 父类{
...
// 子类 需变为模板
template<class T1,class T2>
class 子类:public 父类<T2>		// 将 父类的 T类型 变为 T2 类型
{
	T1 obj;
};

int main(){
	// 实例化 子类对象 时，需 给出 父类和子类的类型
	子类<类型1,类型2> 对象名;

	return 0;
}
```
3. 总结： 如果父类是类模板，子类 需要指定出 父类中T的 数据类型
#### 8.3.6 类模板 成员函数 的 类外实现
1. 语法：**`template<class T>;				类名<T>::类名(T 参数)`**
```c
// 定义 类模板
template<class T1,class T2>
class 类
...
// 类模板 构造函数 的 类外实现
template<class T1,class T2>				// 必须有 模板的参数列表
类<T1,T2>::类(T1 变量，T2 变量){
...
```
2. 总结： 类模板 成员函数 类外实现 时，需要加上 **模板的参数列表**
#### 8.3.7 类模板 成员函数 分文件 编写
1. 产生的问题： 类模板成员函数 创建时 是在 调用阶段，会导致 **分文件编写 时 链接不到**
  - 类模板 成员函数 一开始 是 不创建的

2. 解决：
- 方式1【不常用】：直接包含 .cpp 源文件
`#inclde<Person.cpp>`

- 方式2【常用】：==将 **.h的声明 和 .cpp的实现 写到 同一个文件中，后缀为`.hpp`**==

3. 注意：**`.hpp` 一般是 类模板 的 后缀名（约定俗成）**

#### 8.3.8 类模板 与 友元
1. 学习目标： 掌握 类模板 配合 友元函数 的 类内实现和类外实现
2. 友元 的 实现：
- 全局函数 **类内实现**：==直接在 类内声明 友元 即可==
```c
template<class T1,class T2>
class Person
{
...
```
- 全局函数 **类外实现**：（需 类内声明）需要提前==让 编译器 知道 全局函数 的 存在==
  - 注意：
第一步： 需 **提前让 编译器 知道 类的存在**
```c
// 提前让 编译器 知道 Person类 存在
template<class T1,class T2>
class 类名;
```
第二步：将 全局函数**类外实现 写在 类定义 之前**
```c
// 全局函数 类外实现（需 类内声明）
// 全局函数 不需要 加 作用域；但 需要 模板参数
template<class T1,class T2>
void 全局函数名(Person<T1,T2> &p){     // 这是一个 函数模板 的 实现/定义
    cout << "类外实现 " << endl;
}
```
第三步：在 类模板定义 内部声明 友元 全局函数【需 含有 空模板 列表】
```c
template<class T1,class T2>
class 类名
{
	private:
		// 在 类内声明 全局函数
		// 需 声明 这是一个 模板函数： 加 空模板 的 参数列表
		friend void 全局函数名<>(类名<T1,T2> &p);
};
```
3. 注意：建议 全局函数 做 **类内实现**，用法简单，编译器 直接识别
...
## VS 的 开发人员命令提示符
### 1. 查看 类的内部内容 以及 虚函数表：
1. 进入 工程文件：`工程盘:`，`cd 工程路径`
2. 使用：`cl /dl reportSingleClassLayout类名 "cpp文件名.cpp"`查看
